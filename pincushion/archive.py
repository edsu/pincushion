...
class Generator:
    ...
    def download_media(self):
        self.fetch_file(self.data["user"]["image"], "images/user.jpg")
        collections = list(self.collections())
        for coll in tqdm.tqdm(collections, desc="{:20}".format("collection media")):
            if coll["image_url"]:
                self.fetch_file(
                    coll["image_url"], f"images/collections/{coll['slug']}.jpg"
                )
        for pin in tqdm.tqdm(self.data["pins"], desc="{:20}".format("pin media")):
            if pin["type"] == "photo":
                self.fetch_file(
                    pin["display"]["content"], f"images/pins/{pin['id']}.jpg"
                )
            elif pin["type"] == "audio":
                self.fetch_media(
                    pin["display"]["content"], f"audio/pins/{pin['id']}.mp3"
                )
            elif pin["type"] == "video":
                self.fetch_media(
                    pin["display"]["content"], f"video/pins/{pin['id']}.mp4"
                )
